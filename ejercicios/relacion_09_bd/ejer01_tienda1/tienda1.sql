...
CREATE USER 'alumno' @localhost IDENTIFIED BY 'velazquez';

GRANT USAGE ON *.* TO 'alumno' @localhost IDENTIFIED BY 'velazquez';

GRANT ALL privileges ON `dwes`.* TO 'alumno' @localhost;
...
create view unds_nombre as 
select unidades, nombre_corto  
from producto, stock 
where producto.cod = stock.producto;

create view unds_tienda as 
select unidades, nombre from stock, tienda where tienda = cod;

select * from unds_nombre right join unds_tienda on unds_nombre.unidades = unds_tienda.unidades;
